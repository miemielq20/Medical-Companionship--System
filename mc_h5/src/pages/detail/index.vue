<template>
    ...
    <div class="tips">
        <div class="dzf" v-if="data.trade_state === '待支付'">
            ...
            <div class="text2">
                请在
                <Countdown :time-end="data.time_end" @expired="handleOrderExpired" />
                内支付，超时将自动取消订单
            </div>
        ...
        <div class="dzf" v-if="data.trade_state === '待服务'">
            ...
            <div class="text2">请保持手机通畅，稍后将有专员与您联系</div>
        </div>
...
<script lang="ts" setup>
    ...
    import { type ApiResponse, type orderDetailResponse } from '@/types/response';
    ...
    const loadOrderDetail = () => {
        proxy.$api.orderDetail({ oid: route.query.oid }).then((res: ApiResponse<orderDetailResponse>) => {
            ...
            data.value.time_end = data.value.time_end || data.value.order_start_time + 7200000;
            generateInfoGroups();
        });
    };

    const handleOrderExpired = () => {
        data.value.trade_state = '已取消';
        data.value.service_state = '已取消';
        loadOrderDetail();
    };
